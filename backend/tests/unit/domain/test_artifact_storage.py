...
def test_register_existing_rejects_path_traversal_namespace(tmp_path) -> None:
    source = tmp_path / "source.txt"
    source.write_text("artifact")
    storage = ArtifactStorage(tmp_path / "artifacts")

    with pytest.raises(ValueError, match="single safe path segment"):
        storage.register_existing(source, AssetKind.LOG, "../../escape")
...
def test_register_existing_does_not_overwrite_target_when_same_filename_reimported(tmp_path) -> None:
    storage = ArtifactStorage(tmp_path / "artifacts")
    ...
    assert first.path.name == "source.log"
    assert second.path.name == "source__2.log"
    assert first.path.read_text() == "first"
